...
There are other ways to define the optimal number of factors that are explained here:

THEOS ADD LINK TO ARTICLE
...
- Orthogonal - which assumes that factors are completely uncorrelated (e.g. varimax rotation method)
- Oblique - when the factors are allowed to be correlated (e.g. Oblimin rotation method).
...
Below are the tables of factor loadings and other statistics for the principal axis factoring and two rotation scenarios:
...
The factor scores are useful because they allow comparing customers based on few attitude dimensions (in this case 5 of them). For creating market segments, we will use these five derived variables (constructs) instead of the 19 raw attitudinal variables:

THEOS SHOW FACTOR SCORES
