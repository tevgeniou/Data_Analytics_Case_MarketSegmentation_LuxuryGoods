...
THEOS: Replace variable number with actual question text (i sent you the excel file=)
Could you do this here for the matrix but also going forward? E.g. factor loadings?
...
THEOS: Could you sort loadings so that is easier to read? Also highlight the loadings over 0.4 in green. 
...
THEOS: show a list of the 10 statements with the highest factor loading in each factor. 
...
The figure left is the Dendrogram. The one on the right displays the discriminant coordinates when we define the 5 groups:
...
THEOS SHOW Segment profiles using some examples variables
...
THEOS SHOW full profile for the 5 segment solution
